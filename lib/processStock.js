 ...
 *   HARGA JUAL = ROUND(nstdprice * 1.11, 0)
 *            -> nstdprice (harga standar dari ERP) ditambah PPN 11%, dibulatkan
 *               ke rupiah terdekat.
...
function processStock(saldoRows, supplierRows, kategoriRows) {
  ...
  const rows = saldoRows
    ...
    .map((r, idx) => {
      ...
      // Harga jual = harga standar (nstdprice) dari ERP + PPN 11%, dibulatkan ke rupiah.
      const nstdprice = Number(r.nstdprice) || 0;
      const hargaJual = Math.round(nstdprice * (1 + PPN_RATE));
