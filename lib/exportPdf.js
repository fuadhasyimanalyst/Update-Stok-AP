 ...
 */
export function exportStockToPdf({ rows, columns, asOfDate, stats, pageTitle }) {
  ...
  doc.text(`Update Stok${pageTitle ? " — " + pageTitle : " — Laporan"}`, margin, 36);
  ...
  const body = rows.map((r) =>
    columns.map((c) => {
      ...
      if (c.key === "NSTDPRICE" || c.key === "HARGA_JUAL" || c.key === "NILAI_STOK")
        return `Rp ${Number(v || 0).toLocaleString("id-ID")}`;
      if (c.key === "HARGA_SUMBER") return v === "FALLBACK_ERP" ? "Fallback ERP" : "Daftar Harga";
      return v ?? "-";
    })
  );
  ...
  const slug = pageTitle ? "-" + pageTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-") : "";
  doc.save(`update-stok${slug}${asOfDate ? "-" + asOfDate : ""}.pdf`);
}
